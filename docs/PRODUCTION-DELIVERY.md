...
- 本次核對來源：`c47efa03de396b2dafd4a64b654950e4a7d41f98`。
- 公開網站：GitHub Pages；本次讀回的最新建置為上述提交，狀態 `built`。
- Apps Script：已驗證的同一個正式專案，現有部署說明為 `Production: desktop tenant detail and contract read session repair (main 662e20a)`，版本 **187**。專案及部署指紋見 `production-baseline.json`。
- 實際後端：唯讀匯出版本 187 的 56 個檔案，與 `main/apps-script/` 逐位元組一致。
...
本次 Gate 0 的現行來源、部署與 schema 盤點已完成：56 個後端檔案一致、76 個工作表欄名已記錄、216 項既有回歸通過、11 個公開資產逐位元組相符。已確認正式入口呈現 Email 登入畫面，未要求或代填驗證碼。這是可重現的 V2 internal-beta 來源基線；正式帳號登入後的交易及真實 iPhone／LINE 操作尚未驗收，不能以此宣稱完整營運驗收通過。

報修工單仍是下一個交付項目，不能把本次來源核對當作工單功能完成。已確認規則：房間維修歷史持續保留；房客僅能存取自己租期內獲授權的工單，前房客姓名、聯絡方式、描述中的個資及附件不得因換租而對新房客公開。正式表已有 `V2_tenant_messages`，但沒有 `V2_maintenance_tickets`、`V2_maintenance_ticket_updates`、`V2_maintenance_attachments` 或 `V2_vendors`；現有訊息表也沒有 `contract_id`，不能直接把整間房的歷史訊息提供給新房客。下一階段需以房間保留歷史、以租約隔離房客存取，先完成隔離權限測試再做 additive-only schema migration。
...
本次不更動後端及業務資料。若要撤回本次交付文件與檢查工具，回復本次 Git 提交即可；Apps Script 保持版本 187。將來後端變更前應匯出並保留當時服務版本作為該次 rollback 目標。
