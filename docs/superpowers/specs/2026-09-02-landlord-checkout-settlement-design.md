...
結算表保存計算快照與來源 ID；不複製或改寫原始帳單金額。快速結案的退款確認代表
房東已完成該合約帳務結清，因此後端會將同一 Workspace、房客、房間與合約範圍內的未繳
`V2_bills.payment_status` 更新為既有正式銷帳使用的 `paid`，並同步帳單檢視與 Workspace
摘要；已繳帳單與其他合約不受影響。完整電表結算仍只讀取指定的上月電費／設備費來源，
不因本規則自動變更帳單。
...
- `landlord_contract_checkout_complete`：快速結案要求手動應收／退款金額但不要求電表附件；完整電表結算要求兩個附件 ID。後端重算、append 結算紀錄、同步快速結案範圍內未繳帳單，再執行原退房生命週期更新；相同 idempotency key 回傳相同結果。
...
- 任一驗證、附件或結算寫入失敗時，不得完成退房；原始合約與帳單金額不受影響。快速結案帳單狀態同步採既有 `paid` 規則並可冪等補齊。
...
6. 結算紀錄 append、原始帳單金額／合約不可覆寫；快速結案只同步同合約未繳帳單為 `paid`，退房更新仍保持 idempotent。
...
- 不修改既有月租帳單的原始金額；完整電表結算不自動改變付款狀態，快速結案則將同合約退款結清範圍內的未繳帳單標記為 `paid`。
