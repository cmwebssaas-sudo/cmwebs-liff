...
- [x] 202 清除金額操作具備每筆帳單的處理中／完成鎖定；寫入 API 不自動重送，成功後顯示明確完成提示，回應逾時只做一次唯讀讀取確認並告知是否已完成（Phase 230；本地 UI、validator、Apps Script syntax、完整 Node `132/132` 通過，尚未發布 Pages／手機／LIFF UAT 待驗證）
