...
| 2026-09-18 | One-time test bill archival formal deployment | User explicitly requested `部署正式網站` after confirming the one-time archive design. PR #169 merged as `257093b6`; Apps Script Version 190 serves the existing Web App deployment and Version 189 remains rollback; GitHub Pages workflow `35339276140` completed successfully. The bounded action is available only for closed-room unpaid bills without payment records, preserves history, is idempotent, and does not create payments, send LINE, or delete rows. No Sheet business-row write, Drive, Properties, Trigger, payment, or LINE action was run; authenticated landlord/mobile/LIFF UAT remains `HUMAN_REQUIRED` / `UNVERIFIED`. |
| 2026-09-18 | Test bill archive success-feedback formal deployment | User explicitly requested `部署正式網站` for the missing success signal after one-time bill archival. PR #171 merged as `31515af`; GitHub Pages workflow `35344387333` completed successfully. This was a frontend-only release: Apps Script Version 190 remains serving, Version 189 remains rollback, and the prior Pages main commit `f9dc5fd` is the Pages rollback target. Public HTTP 200 read-back confirmed the in-page `archiveSuccessNotice`, `作廢成功`, and accessible status marker. No Sheet business-row write, Drive, Properties, Trigger, payment, or LINE action was run; authenticated landlord/mobile/LIFF UAT remains `HUMAN_REQUIRED` / `UNVERIFIED`. |
