...
function resolveTenantRequestLineUserId_(
  request,
  action,
  fallbackLineUserId
) {
  ...
  if (!testLineUserId) {
    throw new Error(
      'TEST_TENANT_LINE_UID is empty'
    );
  }

  return testLineUserId;
}


function getTenantReadBindingState_(lineUserId) {
  const result =
...
function getTenantHomeByLineUid(lineUserId) {
  ...
  try {
    ...
    if (
      ...
      const bindingState =
        getTenantReadBindingState_(
          lineUserId
        );
      let code =
        runtimeIdentity && runtimeIdentity.code
          ? runtimeIdentity.code
          : 'TENANT_RUNTIME_RESOLUTION_ERROR';
      let message =
        runtimeIdentity && runtimeIdentity.message
      ...
      logLiffAccess_({
        lineUserId,
        userId:
          bindingState.tenant.user_id || '',
        role: 'tenant',
        action,
        targetId:
          bindingState.tenant.tenant_id || '',
        result: 'failed',
        errorMessage: code
      });
    ...
    if (
      ...
      logLiffAccess_({
        lineUserId,
        userId: canonical.tenant_user_id,
        role: 'tenant',
        action,
        targetId: canonical.tenant_id,
        result: 'failed',
...
function tenantBillsRuntimeText_(value) {
  return String(
...
function tenantBillsRuntimeUpper_(value) {
  return tenantBillsRuntimeText_(value)
    .toUpperCase();
}


function tenantBillsRuntimeFirst_(row, keys) {
  row = row || {};
...
function tenantBillsRuntimeError_(code, message) {
...
function tenantBillsRuntimeRequireOne_(
  rows,
  missingCode,
  duplicateCode,
  label
...
function tenantBillsRuntimeContractActive_(contract) {
  const status =
...
function tenantBillsRuntimeFormatMonth_(value) {
  if (
...
function tenantBillsRuntimeNumber_(value) {
  const number = Number(
    tenantBillsRuntimeText_(value)
      .replace(/,/g, '')
  );
...
function tenantBillsRuntimeIdentityRows_(
  rows,
  identity
) {
  const workspaceId =
...
function tenantBillsRuntimeResolveIdentity_(
  lineUserId
) {
  lineUserId =
    tenantBillsRuntimeText_(
    ...
    contracts.filter(function (contract) {
      return (
        tenantBillsRuntimeUpper_(
          ...
          tenantBillsRuntimeUpper_(tenantId) &&
        tenantBillsRuntimeContractActive_(
          contract
        )
      );
    });
  ...
  if (
    ...
    contractMatches =
      contractMatches.filter(function (contract) {
        return (
          tenantBillsRuntimeText_(
  ...
  [
    ...
    if (normalized) {
      workspaceIds[normalized] = true;
    }
  });

  if (Object.keys(workspaceIds).length !== 1) {
    throw tenantBillsRuntimeError_(
      Object.keys(workspaceIds).length === 0
        ? 'WORKSPACE_ID_MISSING'
        : 'TENANT_BILLS_WORKSPACE_CONFLICT',
      '房客與租約的 Workspace 關聯不完整'
    );
  }

  const workspaceId =
    Object.keys(workspaceIds)[0];
  const contractTenantId =
    tenantBillsRuntimeText_(
      contract.tenant_id
    );
  const contractTenantUserId =
    tenantBillsRuntimeFirst_(
      contract,
      ['tenant_user_id', 'user_id']
    );
  ...
  if (
    ...
    throw tenantBillsRuntimeError_(
      'TENANT_CONTRACT_CONFLICT',
      '租約 tenant_id 與房客主資料不一致'
    );
  }
  ...
  if (
    ...
    contractTenantUserId !== tenantUserId
  ) {
    throw tenantBillsRuntimeError_(
      'TENANT_USER_CONFLICT',
      '租約 tenant_user_id 與房客主資料不一致'
    );
  }
  ...
  return {
    ...
    tenant_id: tenantId,
    tenant_user_id:
      tenantUserId || contractTenantUserId,
    tenant_name:
      tenantBillsRuntimeFirst_(
        tenant,
        ['tenant_name', 'name']
...
function tenantBillsRuntimeRowsByBillId_(rows) {
  const byId = {};
  ...
  (rows || []).forEach(function (row) {
    ...
    const key =
      tenantBillsRuntimeUpper_(billId);
...
function tenantBillsRuntimePublicBill_(
  row,
  identity
) {
  return {
    ...
    bill_month:
      tenantBillsRuntimeFormatMonth_(
        row.bill_month ||
        row.billing_month
      ),
    due_date:
    ...
    electricity_fee_rate:
      tenantBillsRuntimeNumber_(
        row.electricity_fee_rate
      ),
    equipment_fee_rate:
      tenantBillsRuntimeNumber_(
        row.equipment_fee_rate
      ),
    electricity_amount:
      tenantBillsRuntimeNumber_(
...
function tenantBillsRuntimePayload_(lineUserId) {
  ...
  const bills =
    rows
      .map(function (row) {
        return tenantBillsRuntimePublicBill_(
          row,
          identity
        );
      })
...
function tenantBillsRuntimeFailure_(error) {
  const code =
    error && error.code
      ? error.code
      : 'TENANT_BILLS_RUNTIME_ERROR';
  const message =
    error && error.message
      ? error.message
      : '房客帳單資料讀取失敗';
...
function getTenantBillsRuntimePayloadByLineUid_(
  lineUserId
) {
  try {
    return tenantBillsRuntimePayload_(
      lineUserId
    );
  } catch (error) {
    return tenantBillsRuntimeFailure_(
      error
    );
  }
}
...
function getTenantBillsByLineUid(lineUserId) {
  ...
  logLiffAccess_({
    lineUserId:
      tenantBillsRuntimeText_(lineUserId),
    userId:
...
function testGetTenantBillsRuntimePayload() {
  const lineUserId =
    getRequiredScriptProperty_(
      'TEST_TENANT_LINE_UID'
    );
...
function landlordContractHistoryView_(contractRows, tenantRow, landlord) {
  ...
  return contractRenewalHistoryBuildReadModel_(rows, current).map(function (row) {
    return {
      ...
      previous_contract_id: String(row.previous_contract_id || row.renewed_from_contract_id || '').trim(),
