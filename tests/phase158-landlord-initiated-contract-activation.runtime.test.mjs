...

{
  ...
  const duplicateInquiry = runtime.api.landlordInitiatedContractSendRenewalInquiry_(access, created.data.contract.contract_id);
  assert.equal(duplicateInquiry.success, true, duplicateInquiry.code);
  assert.equal(duplicateInquiry.code, 'RENEWAL_REVIEW_ALREADY_CONFIRMED');
  ...
  assert.ok(accepted.data.invite);
